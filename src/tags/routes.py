...
@tags_router.get("/", response_model=List[TagModel],
                 dependencies=[user_role_checker])  # fetches all tags from the database
async def get_all_tags(session: AsyncSession = Depends(get_session)):
    tags = await tag_service.get_tags(session)

    return tags  # returning a list of tags in the response model TagModel
...
async def add_tag(
    ...
    tag_added = await tag_service.add_tag(tag_data=tag_data, session=session)  # add the tag to the database

    return tag_added  # the newly created tag is returned with a 201 Created status
...
async def delete_tag(
    ...
    updated_tag = await tag_service.delete_tag(tag_uid, session)  # removes the tag from the database

    return updated_tag  # returns a 204 No Content status to indicate successful deletion
